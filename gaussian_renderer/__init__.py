...
# ⬇️⬇️⬇️ 新增：eval_jtf_RHS_only 包装函数 ⬇️⬇️⬇️
def eval_jtf_RHS_only(gaussians, forward_output, timing_dict=None, use_double_precision=False):
    # 调用 submodule 中定义的静态方法
    # 参数传递方式完全仿照 eval_jtf_and_get_sparse_jacobian
    return GaussianRasterizer.eval_jtf_RHS_only(
        means3D=gaussians.get_xyz,
        unactivated_rotations=gaussians._rotation,
        unactivated_scales=gaussians._scaling,
        unactivated_opacities=gaussians._opacity,
        shs=gaussians.get_active_features_rest,  # 修正：使用 get_active_features_rest
        params=gaussians.get_reordered_params(with_SH=False), # 修正：使用 get_reordered_params
        forward_output=forward_output,
        use_double_precision=use_double_precision,
        timing_dict=timing_dict
    )
