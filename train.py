...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_from_ply", type=str, default=None,
                        help="Initialize gaussians from a point_cloud.ply and skip SGD init (Kaggle patch).")
    parser.add_argument("--lhs_downsample_scale", type=int, default=1,
                        help="LHS downsampling scale for SSGN (1=no downsample, 2=1/2 resolution, 4=1/4 resolution).")
    ...
    gsgn_args = gn.extract(args)

    if hasattr(args, 'enable_ssgn'):
        gsgn_args.enable_ssgn = args.enable_ssgn
    if hasattr(args, 'enable_fps_lhs'):
        gsgn_args.enable_fps_lhs = args.enable_fps_lhs
    if hasattr(args, 'ssgn_size_rhs') and args.ssgn_size_rhs is not None:
        gsgn_args.ssgn_size_rhs = args.ssgn_size_rhs
    if hasattr(args, 'ssgn_size_lhs') and args.ssgn_size_lhs is not None:
        gsgn_args.ssgn_size_lhs = args.ssgn_size_lhs
    if hasattr(args, 'use_voronoi_weights'):
        gsgn_args.use_voronoi_weights = args.use_voronoi_weights
    if hasattr(args, 'lhs_downsample_scale'):
        gsgn_args.lhs_downsample_scale = args.lhs_downsample_scale
    ...
    dataset_args_for_render = lp.extract(args)
    dataset_args_for_render.model_path = scene.model_path

    for iter in args.save_iterations:
        render_sets(
            dataset_args_for_render,
            iter,
            pp.extract(args),
            skip_test=False,
