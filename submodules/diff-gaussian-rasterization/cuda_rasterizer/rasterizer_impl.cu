...
template<typename T> void CudaRasterizer::Rasterizer::eval_jtf_RHS_only(PackedGSGNDataSpec data, T* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache) {
    data.allocate_pointer_memory();
	CHECK_CUDA(CudaRasterizer::GSGN::eval_jtf_RHS_only<T>(data, r_vec, sparse_jacobians, index_map, per_gaussian_cache), data.debug)
    data.free_pointer_memory();
}
template void CudaRasterizer::Rasterizer::eval_jtf_RHS_only<float>(PackedGSGNDataSpec data, float* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache);
template void CudaRasterizer::Rasterizer::eval_jtf_RHS_only<double>(PackedGSGNDataSpec data, double* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache);
