        ...
        eval_jtf_sparse_render_bkwd_kernel(
            ...
            for (int i = 0; i < rounds; i++, toDo -= 32) {
                ...
                for (int j = 0; j < min(32, toDo); j++) {
                    ...
                    // assert(dL_pos >= 0);
                    // assert(dL_pos < dL_offset); 
                    // assert(data.geomBuffer_ptrs[img_id] != nullptr);
...
template<typename T> void CudaRasterizer::GSGN::eval_jtf_RHS_only(PackedGSGNDataSpec& data, T* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache) {
    // alloc additional memory for the intermediate outputs
    T* helper_memory;
    cudaMallocManaged((void**) &helper_memory, data.max_n_visible_gaussians * (2 + 3 + GSGN_NUM_CHANNELS + 6) * sizeof(T));
...
template void CudaRasterizer::GSGN::eval_jtf_RHS_only<float>(PackedGSGNDataSpec& data, float* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache);
template void CudaRasterizer::GSGN::eval_jtf_RHS_only<double>(PackedGSGNDataSpec& data, double* r_vec, __half** sparse_jacobians, int** index_map, float** per_gaussian_cache);


// -------------------------------------------------------------------------
// END: SSGN Optimization
// -------------------------------------------------------------------------
