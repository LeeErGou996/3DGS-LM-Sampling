...
class GaussianRasterizer(nn.Module):
    ...
    def build_gsgn_data_spec(
        ...
        x_padded = pad(x, 16, 2)
        x_padded = pad(x_padded, 2, 1)

        # calculate the sum per warp. one warp is a 2x16 (HxW) patch in the image
        x_unfolded = x_padded.unfold(1, 2, 2).unfold(2, 16, 16)  # (num_images, num_warps_h, num_warps_w, 2, 16)
        x_reshaped = x_unfolded.reshape(num_images, x_unfolded.shape[1], x_unfolded.shape[2], -1)  # (num_images, num_warps_h, num_warps_w, 32)
        x_sum = x_reshaped.sum(dim=-1, dtype=x.dtype)  # (num_images, num_warps_h, num_warps_w)

        # prefix sum goes over the sum per warp
        x_flat = x_sum.view(num_images, -1)
        x_cumsum = torch.cumsum(x_flat, dim=1, dtype=x.dtype)
        ...
        num_sparse_gaussians = [xi.item() for xi in x_cumsum[:, -1]]
        ...
        assert (x_final >= 0).all()

        gsgn_data_spec.n_contrib_vol_rend_prefix_sum = [xi for xi in x_final]
        ...
        max_num_visible_gaussians = max(num_visible_gaussians) if num_visible_gaussians else 0
